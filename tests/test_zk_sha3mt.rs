...
fn zk_sha3mt_w_vpd<T: Field>(log_leaf_size: usize) {
    ...
    println!("sha3-256 merkle tree generated in {} ms", in_ms as f64 / 1000.0);
    ...
    let mut file = OpenOptions::new()
        .write(true)
        .append(true)
        .open(std::format!("log/zk_sha3mt_{}.log", type_name::<T>()))
        .unwrap();
    statistic(&mut file, log_leaf_size, n_ldt_iter, N_MPC_ITER);
}


fn test_zk_sha3mt<T: Field>() {
    ...
    for log_leaf_size in 1..12 {
        println!("num of leaves: {0}", 1 << log_leaf_size);
        println!("log inputs: {0}", log_leaf_size + 10);
        zk_sha3mt_w_vpd::<T>(log_leaf_size);
    }
}

#[test]
fn test_zk_sha3mt_all() {
    println!("");
    // test_zk_sha3mt::<GF2p16>();
    // test_zk_sha3mt::<GF2p32>();
    // test_zk_sha3mt::<GF2p64>();
    // test_zk_sha3mt::<GF2p128>();
    test_zk_sha3mt::<GF2p192>();
    // test_zk_sha3mt::<GF2p256>();
}
