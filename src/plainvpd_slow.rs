...
let start = Instant::now();
    ...
    #[cfg(feature = "microbench")] let start0 = Instant::now();
        let mut h_ex = (self.coset.fft)(self.coset, &h_coeff, self.log_len, self.log_l);
    #[cfg(feature = "microbench")] let elapsed = start0.elapsed();
    #[cfg(feature = "microbench")] let in_ms = elapsed.as_micros() as usize;
    ...
    #[cfg(feature = "microbench")] let start0 = Instant::now();
        ...
        for i in 0..(1 << self.log_l) {
            c_ex.push(randoms[0] * lq[i] + randoms[1] * h_ex[i] + randoms[2] * p_ex[i]);
        }
...
let start = Instant::now();
        ...
        let mut ldt_prover = LDTProver::<T, Ch>::new(self.ch, &self.coset, log_degree, 2 * n - 1);
...
let start = Instant::now();
        ...
        for i in 0..self.n_ldt_iter {
            ...
            assert_eq!(c_x, lq_x * randoms[0] + h_x * x_n * randoms[1] + p_x * randoms[2]);
        }
