...
// The zkVPD protocol simplified from Virgo
// with masking and LDT over the quotient polynomial.
// masking polynomial is minimized as degree-(kappa-1) * Z and degree-(kappa * log n)
...
let start = Instant::now();
    ...
    #[cfg(feature = "microbench")] println!("  ┃ ├──────#fft of q(x) {:>2}->{:>2}:             {:9.3} ms", self.log_len, self.log_l, in_ms as f64 / 1000.0);
        ...
        let s_degree = std::cmp::min(n - 2, k * self.log_len - 1);
        for _ in 0..=s_degree {
    ...
    #[cfg(feature = "microbench")] let start0 = Instant::now();
        let mut lq_coeff =
            if T::CHAR == 2 {
                (self.coset.ifft)(self.coset, &lq, self.log_len + log_m)
            } else {
                (self.coset.ifft)(self.coset, &lq, self.log_l)
            };
        lq_coeff.truncate(2 * n + k - 1);
    #[cfg(feature = "microbench")] let elapsed = start0.elapsed();
    #[cfg(feature = "microbench")] let in_ms = elapsed.as_micros() as usize;
    ...
    #[cfg(feature = "microbench")] let start0 = Instant::now();
        ...
        if T::CHAR == 2 {
            for i in 0..(1 << self.log_l) {
                ...
                // p_ex.push(alpha * ((lq[i] - self.coset.vanishing_eval[i] * h_ex[i] - p_n_1 * self.coset.x_n_1[i]) * self.coset.x_n_k[i]) + s_ex[i]);
                p_ex.push(alpha * (lq[i] - self.coset.vanishing_eval[i] * h_ex[i] - p_n_1 * self.coset.x_n_1[i]) + s_ex[i]);
            }
        } else {
            for i in 0..(1 << self.log_l) {
                // p_ex[i] = (p_ex[i] - self.coset.vanishing_eval[i] * h_ex[i] - p_n_1) * self.coset.x_n_1[i] * self.coset.x_n_k[i];
...
impl<'a, T: Field, Ch: Channel<Output = T>> Verifier<'a, T, Ch> {
    ...
    pub fn verify(&mut self, mu: T, l_commit: SHA256, q_circ: &mut Circuit<'a, T>, _q_inputs: &Vec<T>) -> (bool, T) {
        let (_, _, s_commit) = recv!(self.ch, VPDOpen0, 0);
...
let start = Instant::now();
        ...
        for i in 0..self.n_ldt_iter {
            ...
            let p_x =
                if T::CHAR == 2 {
                    alpha * (lq_x - h_x * self.coset.vanishing_eval[x] -
                        mu * x_n_1 * self.coset.ksi) + s_x
                } else {
                    alpha * (lq_x - h_x * self.coset.vanishing_eval[x] -
                        mu * self.coset.ksi) * x_n_1 + s_x
                };
