...
fn zk_ringsig_w_vpd<T: Field>(log_ring_size: usize) {
    ...
    let mut file = OpenOptions::new()
        ...
        .create(true)
        .append(true)
        .open(std::format!("log/zk_ringsig_p{}_vith_{}.log", N_PARTY, type_name::<T>()))
        .unwrap();
...
fn test_zk_ringsig<T: Field>() {
    ...
    //     .append(false)
    //     .truncate(true)
    //     .open(std::format!("log/zk_ringsig_vith_{}.log", type_name::<T>()))
    //     .unwrap();
...
fn main() {
    ...
    println!("ring size: 2^{0}", log_ring_size);
    zk_ringsig_w_vpd::<GF2p192>(log_ring_size);
