...
std::vector<uint8_t>
group_phase_1_commitment(const banquet_instance_t &instance,
                   const banquet_salt_t &salt,
...
std::vector<std::vector<field::GF2E>>
phase_sumcheck0_expand(const banquet_instance_t &instance,
...
phase_sumcheck_expand(const banquet_instance_t &instance,
        const std::vector<uint8_t> &h_1) {
  hash_context ctx;
  hash_init(&ctx, instance.digest_size);
  hash_update(&ctx, h_1.data(), h_1.size());
...
std::vector<std::vector<field::GF2E>>
phase_sumcheck_final_expand(const banquet_instance_t &instance,
        const std::vector<uint8_t> &h_1) {
    hash_context ctx;
    hash_init(&ctx, instance.digest_size);
    hash_update(&ctx, h_1.data(), h_1.size());
...
std::vector<uint8_t>
phase_group1_expand(const banquet_instance_t &instance,
...
std::vector<uint8_t>
phase_sumcheck_commitment(const banquet_instance_t &instance,
                   const banquet_salt_t &salt, const std::vector<uint8_t> &h,
...
std::vector<uint8_t>
phase_sumcheck_final_commitment(const banquet_instance_t &instance,
                   const banquet_salt_t &salt, const std::vector<uint8_t> &h,
...
void gen_proof(const banquet_instance_t &instance, const Circuit<field::GF2E> C, std::vector<uint8_t> inputs) {
...
/* banquet_group_signature_t banquet_group_sign(
        const banquet_instance_t &instance,
        const banquet_keypair_t &keypair,
  ...
  /////////////////////////////////////////////////////////////////////////////
    ...
    // build signature
    std::vector<banquet_repetition_group_proof_t> proofs;
    ...
    for (size_t e = 0; e < instance.num_rounds; e++) {
        ...
        banquet_repetition_group_proof_t proof{
            seeds[e],
            commitment,
            rep_key_deltas[e],
            rep_pt_deltas[e],
    ...
    banquet_group_signature_t signature{salt, h_1, h_5, proofs};
...
bool banquet_group_verify(const banquet_instance_t &instance,
  ...
  // recompute h_2
  std::vector<std::vector<field::GF2E>> P_deltas;
  for (const banquet_repetition_group_proof_t &proof : signature.proofs) {
    P_deltas.push_back(proof.P_delta);
  }
  std::vector<uint8_t> h_2 =
      phase_2_commitment(instance, signature.salt, h_i, P_deltas);
  ...
  // rebuild SeedTrees
  for (size_t repetition = 0; repetition < instance.num_rounds; repetition++) {
    const banquet_repetition_group_proof_t &proof = signature.proofs[repetition];
    // regenerate generate seed tree for the N parties (except the missing
    // one)
    if (missing_parties[repetition] != proof.reveallist.second)
      throw std::runtime_error(
...
banquet_serialize_group_signature(const banquet_instance_t &instance,
        size_t gsize, const banquet_group_signature_t &signature) {
